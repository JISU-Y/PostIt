...
(function (window, factory) {
  ...
  if (typeof define == "function" && define.amd) {
    // AMD
    define(["ev-emitter/ev-emitter", "unipointer/unipointer"], function (
      EvEmitter,
      Unipointer
    ) {
    ...
    // browser global
    window.Huebee = factory(window, window.EvEmitter, window.Unipointer);
  }
  ...
  proto.create = function () {
    ...
    element.className += this.options.staticOpen
      ? "is-static-open "
      : "is-hidden ";
  ...
  proto.updateColorModer = function () {
    this.colorModer =
      colorModers[this.options.notation] || colorModers.shortHex;
  };
  ...
  proto.updateSizes = function () {
    ...
    this.cursorBorder = parseInt(
      getComputedStyle(this.cursor).borderTopWidth,
      10
    );
  ...
  proto.setSwatch = function (swatch) {
    ...
    var lightness =
      this.lum - Math.cos(((this.hue + 70) / 180) * Math.PI) * 0.15;
  ...
  function htmlInit() {
    ...
    for (var i = 0; i < elems.length; i++) {
      ...
      try {
        ...
        if (console) {
          console.error(
            "Error parsing data-huebee on " + elem.className + ": " + error
          );
        }
        continue;
      }
  ...
  function rgb2hsl(r, g, b) {
    ...
    var h;
    if (C === 0) h = 0;
    // spec'd as undefined, but usually set to 0
